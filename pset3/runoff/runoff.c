...
int find_min(void)
{
    ...
    for (int j = 0; j < candidate_count; j++)
    {
        if (candidates[j].votes < voteCount && candidates[j].eliminated == false)
        {
            voteCount = candidates[j].votes;
        }
    }
    return voteCount;
}
...
{
    // TODO
    //a tie happens if every candidate still in the election has the same number of votes
    for (int j = 0; j < candidate_count; j++)
    {
